...
class TestVectorStore:
    ...
    def test_max_results_configuration_issue(self):
        ...
        config = Config()
        assert config.MAX_RESULTS == 0, "Expected MAX_RESULTS to be 0 based on the current config"
        
        # Create vector store with problematic config
        store = SimpleVectorStore(config.CHROMA_PATH, config.EMBEDDING_MODEL, config.MAX_RESULTS)
    ...
    def test_search_with_zero_max_results(self):
        ...
        config = Config()
        store = SimpleVectorStore(config.CHROMA_PATH, config.EMBEDDING_MODEL, 0)  # Explicitly set to 0
    ...
    def test_search_with_valid_max_results(self):
        ...
        config = Config()
        store = SimpleVectorStore(config.CHROMA_PATH, config.EMBEDDING_MODEL, 5)  # Use valid setting
    ...
    def test_search_with_explicit_limit_override(self):
        ...
        config = Config()
        store = SimpleVectorStore(config.CHROMA_PATH, config.EMBEDDING_MODEL, 0)  # Problematic setting
        
        # Try to override with explicit limit
        results = store.search("MCP server creation", course_name="MCP", limit=5)
        
        print(f"Search results with MAX_RESULTS=0 but limit=5: {len(results.documents)} documents")
