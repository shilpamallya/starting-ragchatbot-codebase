...
class TestCourseSearchTool:
    ...
    def test_execute_with_results(self, course_search_tool, sample_search_results):
        ...
        assert course_search_tool.last_sources[0]['title'] == "MCP: Build Rich-Context AI Apps with Anthropic"
    ...
    def test_execute_with_search_error(self):
        ...
        mock_store = Mock()
        error_results = SearchResults(documents=[], metadata=[], distances=[], error="Search failed")
    ...
    def test_execute_with_lesson_number(self, course_search_tool):
        ...
        result = course_search_tool.execute("server", course_name="MCP", lesson_number=4)
...
class TestCourseSearchToolIntegration:
    ...
    def test_real_vector_store_search_with_fixed_config(self):
        ...
        config = Config()
        store = SimpleVectorStore(config.CHROMA_PATH, config.EMBEDDING_MODEL, 5)  # Fixed MAX_RESULTS
    ...
    def test_lesson_specific_search_real(self):
        ...
        config = Config()
        store = SimpleVectorStore(config.CHROMA_PATH, config.EMBEDDING_MODEL, 5)  # Use working config
